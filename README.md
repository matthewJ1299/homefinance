...
- **Lists**: Shared lists (household-wide, visible to all users) and personal lists (per-user only). **Lists** in the nav opens the default list (first by sort order) for the selected scope; use the shared/personal toggle to switch scopes. Create and delete lists under **Settings** > **Lists**. On a list’s page, the list switcher jumps between lists in the same scope. Add items with label and quantity; use plus/minus to change quantity. Click an item to mark it complete (strikethrough, moves to bottom). Delete individual items or "Delete all completed" for a list.
