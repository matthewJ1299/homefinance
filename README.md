...
- **AI expense analysis (optional)**: Optional AI module for monthly budget analysis (dashboard + Summary). Configure **Free AI** and/or **Paid AI** via env vars; each user can toggle which tier to use under **Settings** > **AI analysis**. **Analyze spending** sends a structured JSON payload to Google Gemini (amounts in **cents**, ZAR, category totals including **previous month spent per category**, plus a **transactions** list in cents for recategorisation hints). The model is instructed to return **only JSON** matching the budget-coach schema; the app parses it and opens **`/budget-ai-report`** (data held in **session storage** for that browser tab session—see [docs/ai-budget-analysis.md](./docs/ai-budget-analysis.md)). Runs are still logged to `ai_analysis_runs` (`input_json`, full prompt text, raw model output, `prompt_template_id` / `prompt_version`). Rate-limited to 5 calls per user per hour.
