...
- **Registration is admin-only**: removed the public `/register` page and `registerNewHouseholdAction`. New households/users are created from `/admin` (`AdminUserProvisioningService`, which now also runs `bootstrapHouseholdDefaults`). The login page and `authorized()` middleware no longer reference `/register`.
