...
- **Provisioning** — Public self-registration is **disabled**. A global super-admin creates households and users from the **admin portal** (`/admin`); `AdminUserProvisioningService.createHouseholdWithOwnerUser` also runs `bootstrapHouseholdDefaults` (default split group + categories + calendar categories).
...
- Migrations: `drizzle/0027_households_pg.sql`, `drizzle/0028_super_admin_and_household_feature_policy_pg.sql`, `drizzle/0029_users_setup_wizard_state_pg.sql` (manifest: `src/lib/db/migration-manifest.ts`, detection: `src/lib/db/migration-seed.ts`)
...
- Provisioning: `src/lib/services/admin/admin-user-provisioning.service.ts`, `src/app/admin/*`
