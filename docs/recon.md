...
Each row has a **Mark** column with a radio group: **None** (no bulk action for that row), **Ignore**, or **Accept**. Use **Process marked** to apply: ignores run first; **Accept** on a possible-duplicate row calls **accept duplicate**; **Accept** on a needs-add row calls **accept add** (category and split 50/50 apply). Rows marked **Accept** that still need a category but have none selected are **skipped**; a toast explains how many were left so you can categorize and run again. **Clear marks** sets every row back to **None**. Per-row **Accept as duplicate**, **Accept and add**, and **Ignore** still work as shortcuts.
