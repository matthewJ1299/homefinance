...
Each row has a **Mark** column with a radio group: **None** (no bulk action for that row), **Ignore**, or **Accept**. Rows marked **Ignore** or **Accept** are visually muted. **Amount** defaults to the parsed bank notification amount (ZAR); you can edit it before **Accept and add** or **Process marked** so the new expense uses that value (minor units sent to the API). **Expense note** defaults to `Recon` / `Recon: {vendor}` and is stored on the expense created by **Accept and add** (or bulk accept-add). Use **Process marked** to apply: ignores run first; **Accept** on a possible-duplicate row calls **accept duplicate**; **Accept** on a needs-add row calls **accept add** (category, amount, note, and split 50/50 apply). After a successful bulk run, a **summary** dialog lists the **date range** of processed txn dates, **counts** (accepted vs ignored, with new vs duplicate), **per-category totals** for newly added expenses, and **separate bank-amount totals** for accepted (duplicates + new adds) vs ignored. If any **Split 50/50** rows were newly added in the run, a toast also shows the **total value of split purchases added**. Rows marked **Accept** that still need a category but have none selected are **skipped**; rows with an invalid or non-positive amount are **skipped**; the summary and toasts explain how many. **Clear marks** sets every row back to **None**. Per-row **Accept as duplicate**, **Accept and add**, and **Ignore** still work as shortcuts.
...
| POST | `/api/recon/items/[id]/accept-add` | Body `{ categoryId, accountId?, split?, note?, amount? }` — create expense (optionally split). Optional `amount` (positive integer, minor units); if omitted, the amount on the recon row is used. Optional `note` (max 500); if omitted, default is `Recon` or `Recon: {vendor}`; empty string clears the note. |
