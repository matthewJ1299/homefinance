...
Each row has a **Mark** column with a radio group: **None** (no bulk action for that row), **Ignore**, or **Accept**. Rows marked **Ignore** or **Accept** are visually muted. Use **Process marked** to apply: ignores run first; **Accept** on a possible-duplicate row calls **accept duplicate**; **Accept** on a needs-add row calls **accept add** (category and split 50/50 apply). After a successful bulk run, a **summary** dialog lists the **date range** of processed txn dates, **counts** (accepted vs ignored, with new vs duplicate), **per-category totals** for newly added expenses, and the **grand total** of processed bank amounts. Rows marked **Accept** that still need a category but have none selected are **skipped**; the summary and a toast explain how many. **Clear marks** sets every row back to **None**. Per-row **Accept as duplicate**, **Accept and add**, and **Ignore** still work as shortcuts.
