...
export async function renameHousehold(name: string): Promise<HouseholdActionResult> {
  return authedAction<{ success: true }>(async () => {
    ...
    return { success: true };
  }, { onError: "That name didn't save. Try again." });
}
...
export async function setHouseholdBudgetMonthStartDay(
  ...
  return authedAction<{ success: true }>(async () => {
    ...
    // The memo was populated earlier in this same request by the settings page load.
    ...
    return { success: true };
  }, { onError: "That day didn't save. Try again." });
}
